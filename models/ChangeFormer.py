...
class pixel_CC(nn.Module):
    ...
    def forward(self, x1, x2):
        # x = b, c, h, w
        mu1 = torch.mean(x1, dim=(2,3), keepdim=True)
        mu2 = torch.mean(x2, dim=(2,3), keepdim=True)
...
class DecoderTransformer_v4(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        #convolutional Difference Modules
        self.diff_c4   = pixel_CC(in_channels=self.embedding_dim, out_channels=self.embedding_dim)
        self.diff_c3   = pixel_CC(in_channels=self.embedding_dim, out_channels=self.embedding_dim)
        self.diff_c2   = pixel_CC(in_channels=self.embedding_dim, out_channels=self.embedding_dim)
        self.diff_c1   = pixel_CC(in_channels=self.embedding_dim, out_channels=self.embedding_dim)
...
class ChangeFormerV7(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        super(ChangeFormerV7, self).__init__()
        #Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        self.depths     = [3, 3, 4, 3] #[3, 3, 6, 18, 3]
        self.embedding_dim = embed_dim
