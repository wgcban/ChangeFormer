...
class DecoderTransformer_x2_v2(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3, 4], align_corners=True, 
        ...
        self.linear_c5 = MLP(input_dim=2*c5_in_channels, embed_dim=self.embedding_dim)
        self.linear_c4 = MLP(input_dim=2*c4_in_channels, embed_dim=self.embedding_dim)
        self.linear_c3 = MLP(input_dim=2*c3_in_channels, embed_dim=self.embedding_dim)
        self.linear_c2 = MLP(input_dim=2*c2_in_channels, embed_dim=self.embedding_dim)
        self.linear_c1 = MLP(input_dim=2*c1_in_channels, embed_dim=self.embedding_dim)
    ...
    def forward(self, inputs1, inputs2):
        ...
        #Stage-2 (x1/16 scale)
        _c4 = self.linear_c4(torch.cat((c4_1, c4_2), dim=1)).permute(0,2,1).reshape(n, -1, c4_1.shape[2], c4_1.shape[3])
        p_c4  = self.make_pred_c4(_c4)
        outputs.append(p_c4)
        _c4_up= resize(_c4, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        #Stage-3 (x1/8 scale)
        _c3 = self.linear_c3(torch.cat((c3_1, c3_2), dim=1)).permute(0,2,1).reshape(n, -1, c3_1.shape[2], c3_1.shape[3])
        p_c3  = self.make_pred_c3(_c3)
        outputs.append(p_c3)
        _c3_up= resize(_c3, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        #Stage-2 (x1/4 scale)
        _c2 = self.linear_c2(torch.cat((c2_1, c2_2), dim=1)).permute(0,2,1).reshape(n, -1, c2_1.shape[2], c2_1.shape[3])
        ...
        #Upsampling to input scale through a convolutional layer
        x = self.convd2x(_c)
